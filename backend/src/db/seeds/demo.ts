/**
 * FEAT-09 (docs/feat1.md) — generates a full read-only demo account.
...
async function main() {
  ...
  if (existing) {
    ...
    console.log('Removed previous demo user (cascade).');
  }
  ...
  // --- Collectibles: 8 cards + 3 sealed products ---
  const cards: (typeof schema.collectibleItems.$inferInsert)[] = [
    { userId: user.id, itemType: 'card', name: 'Dracaufeu', setName: 'Écarlate et Violet 151', cardNumber: '006/165', condition: 'NM', purchasePrice: 15_000, purchaseDate: dayInMonth(8, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Mewtwo', setName: 'Obsidian Flames', cardNumber: '051/197', condition: 'NM', purchasePrice: 8_000, purchaseDate: dayInMonth(7, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Pikachu', setName: 'Célébrations', cardNumber: '005/025', condition: 'LP', purchasePrice: 4_000, purchaseDate: dayInMonth(10, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Léviator', setName: 'Écarlate et Violet', cardNumber: '038/198', condition: 'NM', purchasePrice: 3_000, purchaseDate: dayInMonth(6, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Ronflex', setName: 'Évolutions Prismatiques', cardNumber: '143/131', condition: 'NM', purchasePrice: 6_000, purchaseDate: dayInMonth(4, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Évoli', setName: 'Évolutions Prismatiques', cardNumber: '130/131', condition: 'MP', purchasePrice: 2_500, purchaseDate: dayInMonth(4, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Tortank', setName: '151', cardNumber: '009/165', condition: 'NM', purchasePrice: 9_000, purchaseDate: dayInMonth(9, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'card', name: 'Mélodelfe', setName: 'Écarlate et Violet', cardNumber: '196/198', condition: 'NM', purchasePrice: 3_500, purchaseDate: dayInMonth(5, 1).toISOString().slice(0, 10), priceSource: 'manual' },
  ];
  const sealed: (typeof schema.collectibleItems.$inferInsert)[] = [
    { userId: user.id, itemType: 'sealed', name: 'Booster Box — Écarlate et Violet 151', sealedType: 'booster_box', sealedLanguage: 'FR', purchasePrice: 25_000, purchaseDate: dayInMonth(8, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'sealed', name: 'Elite Trainer Box — Obsidian Flames', sealedType: 'etb', sealedLanguage: 'EN', purchasePrice: 6_000, purchaseDate: dayInMonth(7, 1).toISOString().slice(0, 10), priceSource: 'manual' },
    { userId: user.id, itemType: 'sealed', name: 'Display — Évolutions Prismatiques', sealedType: 'display', sealedLanguage: 'FR', purchasePrice: 40_000, purchaseDate: dayInMonth(4, 1).toISOString().slice(0, 10), priceSource: 'manual' },
  ];
  const insertedCollectibles = await db.insert(schema.collectibleItems).values([...cards, ...sealed]).returning();
  await db.insert(schema.collectiblePriceSnapshots).values(
    insertedCollectibles.map((item) => ({
      itemId: item.id,
